...
{
    public partial class FormMain : Form, IFacebookDataObserver
    {
        private const string k_AppId = "1783124789311728";
        private const string k_DesignPatternsToken = "b77a5c561934e089";
        ...
        //Observer (Subject), Proxy (cache), Facade (FacebookManager)
        ...
        private void runPhotoAnalysis(PhotoFeatureCreator i_Creator)
        {
            ...
            worker.DoWork += (s, e) =>
            {
                e.Result = timedFeature.Execute(e.Argument as User);
            };

            worker.RunWorkerCompleted += (s, e) =>
            {
                m_ButtonAnalyzePhoto.Enabled = true;
                m_LabelAnalysisTime.Text = $"Analysis time: {timedFeature.LastElapsed.TotalSeconds:F2}s";
                ...
                if (safeFeature != null && safeFeature.HadError)
                {
                    m_LabelPhotoResultDetails.Text = $"Analysis blocked by Safe Decorator:\n{safeFeature.LastError}";
                    return;
                }
        ...
        {
            m_PictureBoxPhotoResult.ImageLocation = safeUrl(i_Photo.PictureNormalURL);
        ...
        private static string safeUrl(string i_Url)
        {
            return string.IsNullOrEmpty(i_Url) ? null : i_Url;
        }
        ...
        private void tryPostStatus(string i_Content)
        {
            try
            {
                ...
                // Observer pattern: PostStatus notifies Subject → OnFacebookDataChanged(PostPublished) → loadRecentPosts()
            }
            catch (Exception ex)
            {
                MessageBox.Show(
        ...
        private void showSelectedAlbum(Album i_Album)
        {
            ...
            foreach (Photo photo in photos)
            {
                ...
                var pic = new PictureBox
                {
                    ...
                    ImageLocation = safeUrl(photo.PictureNormalURL),
            ...
            using (OpenFileDialog dialog = new OpenFileDialog())
            {
                ...
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        // Observer pattern: UploadPhotoToAlbum notifies Subject → OnFacebookDataChanged(PhotoUploaded) → updates album count
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(
        ...
        {
            m_PictureBoxFriendDetail.ImageLocation = safeUrl(i_Friend.PictureNormalURL);
        ...
        private void afterLogin()
        {
            ...
            if (user != null)
            {
                pictureBoxProfile.ImageLocation = safeUrl(user.PictureNormalURL);
        ...
        private void buttonLogout_Click(object sender, EventArgs e)
        {
            ...
            // Observer pattern: Logout notifies Subject → OnFacebookDataChanged(LoggedOut) → clearUserData()
        ...
        // Observer Pattern — FormMain is the Concrete Observer
        public void OnFacebookDataChanged(eFacebookDataChangeType i_ChangeType)
        {
            switch (i_ChangeType)
            {
                case eFacebookDataChangeType.PostPublished:
                    loadRecentPosts();
                    break;
                case eFacebookDataChangeType.PhotoUploaded:
                    if (m_CurrentSelectedAlbum != null)
                    {
                        int albumIndex = r_LoadedAlbums.IndexOf(m_CurrentSelectedAlbum);

                        if (albumIndex >= 0 && albumIndex < m_AlbumBindingList.Count)
                        {
                            m_AlbumBindingList[albumIndex].PhotoCount =
                                r_FacebookService.GetPhotosFromAlbum(m_CurrentSelectedAlbum).Count;
                        }
                    }
                    break;
                case eFacebookDataChangeType.LoggedOut:
                    clearUserData();
                    break;
            }
        }
        ...
        private void buttonOldestPhoto_Click_1(object sender, EventArgs e)
        {
            if (ensureLoggedIn())
            {
                ...
                if (photo != null)
                {
                    pictureBoxOldestPhoto.ImageLocation = safeUrl(photo.PictureNormalURL);
        ...
        private void buttonMostLikedPhoto_Click_1(object sender, EventArgs e)
        {
            if (ensureLoggedIn())
            {
                ...
                if (photo != null)
                {
                    pictureBoxMostLikePhoto.ImageLocation = safeUrl(photo.PictureNormalURL);
                }
                else
                {
        ...
        private void buttonMostCommentedPhoto_Click_1(object sender, EventArgs e)
        {
            if (ensureLoggedIn())
            {
                ...
                if (photo != null)
                {
                    pictureBoxMostCommentedPhoto.ImageLocation = safeUrl(photo.PictureNormalURL);
